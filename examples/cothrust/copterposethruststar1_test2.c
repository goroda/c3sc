       ...
       double * jac, void * args)
{
    ...
    out[order[1]] = x[4];
    out[order[2]] = x[5];
    out[order[3]] = tth*(u[0]-mg)/m;
    out[order[4]] = -tphi/cth*(u[0]-mg)/m;
    out[order[5]] = g + (u[0]-mg)/m;
    ...
    if (jac != NULL){
        ...
        jac[order[2]+6]  = 0.0;
        jac[order[3]+6]  = 0.0;
        jac[order[4]+6] = -1/pow(cphi,2)/cth*(u[0]-mg)/m;
        jac[order[5]+6] = 0.0; 

        jac[order[0]+12] = 0.0;
        jac[order[1]+12] = 0.0;
        jac[order[2]+12] = 0.0;
        jac[order[3]+12] = 1/pow(cth,2)*(u[0]-mg)/m;
        jac[order[4]+12] = -tphi*sth/pow(cth,2)*(u[0]-mg)/m;
        jac[order[5]+12] = 0.0;
    }
              ...
              double * grad)
{
    ...
    *out = 0.0;
    ...
    // controls
    double wu1 = 0.0;//1/400 (a);//0.5;//1/144;//1/170;0.5; //0.5. 10@n8, 10@n16,10@n32
    double wu2 = 0.03; //0.5 // 0.2 (a);   //2
    double wu3 = 0.2; //0.5 // 0.2 (a);	//2
    //*out = *out + 10.0 + wu1 * pow(u[0],2) + wu2 * pow(u[1],2) + wu3 * pow(u[2],2); (a)
    *out = *out + 40.0 + wu1 * pow(u[0],2) + wu2 * pow(u[1],2) + wu3 * pow(u[2],2); //30 //40

    /* *out = *out + 8.0*pow(x[3]-1.0,2.0); */
    *out = *out + 8.0*pow(x[2],2.0);
    *out = *out + 2.0*pow(x[1],2.0);
...
int main(int argc, char * argv[])
{
    ...
    approx_args_set_cross_tol(aargs,1e-3); //decreased to e-7 at some point
    approx_args_set_round_tol(aargs,1e-6); //decrtease to e-6 at some point
...
//    printf("initialized integrator\n");
	...
	//double state[6] = {-3.0, -1.0, 0.2, 0.0, 0.0, 0.0};
